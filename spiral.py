...
def render_set(args, name, iteration, views, gaussians, pipeline, background, near):
    render_path = os.path.join(args.model_path, name, "ours_{}".format(iteration))
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress", ascii=True, dynamic_ncols=True)):
        ...
        depth_est = depth.squeeze().detach().cpu().numpy()
        depth_est = visualize_cmap(depth_est, np.ones_like(depth_est), colormaps.get_cmap('turbo'), curve_fn=depth_curve_fn).copy()
    ...
    os.system(f"ffmpeg -i " + render_path + f"/%5d.png -q 2 " + args.model_path + "/out_{}.mp4 -y".format(scene_name))
    os.system(f"ffmpeg -i " + render_path + f"/depth_%5d.png -q 2 " + args.model_path + "/out_depth_{}.mp4 -y".format(scene_name))
    os.system(f"ffmpeg -i " + render_path + f"/cdepth_%5d.png -q 2 " + args.model_path + "/out_cdepth_{}.mp4 -y".format(scene_name))
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, near):
    with torch.no_grad():
        ...
        render_set(dataset, "render", scene.loaded_iter, scene.getRenderCameras(), gaussians, pipeline, background, near)
